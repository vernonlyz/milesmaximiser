...
Supabase (Postgres + Auth + RLS)
    ...
    ├─ card_mcc_eligibility           — MCC ranges per card interpreted by card_library.mcc_mode: whitelist = these earn bonus, blacklist = these are excluded, hybrid = channel-dependent (per-row payment_channel; migration 053); shared read-only (migrations 044/045/048/049/051/053/054/055)
...
| MCC eligibility seeded — UOB Lady's Card + UOB KrisFlyer Visa + Maybank XL Rewards (whitelist) | Complete |
