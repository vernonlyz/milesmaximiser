...
Browser (React + Vite + TypeScript)
    ...
    ├─ components/        — Shared UI (Layout, Modal, CapUsageBar, StatusBadge, VendorInput, ProtectedRoute, StatementDayPrompt, UpdatePrompt, ExpensesTrends, MilesTabs, DatePicker, PartialBonusNote, ErrorBoundary, MccInfo, MissedMiles, BankBadge, Chip, ThemeToggle, Skeleton, ErrorState)
    └─ lib/
...
Supabase (Postgres + Auth + RLS)
    ...
    ├─ card_mcc_eligibility           — MCC ranges per card interpreted by card_library.mcc_mode: whitelist = these earn bonus, blacklist = these are excluded, hybrid = channel-dependent (per-row payment_channel; migration 053); per-row `reduced` flag = earns a reduced rate not zero (migration 065); shared read-only (migrations 044/045/048/049/051/053/054/055/056/057/058/059/060/061/062/063/064/065/066/067/072/077)
