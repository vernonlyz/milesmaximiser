...
UPDATE card_library SET mcc_mode = 'blacklist' WHERE id IN (
  ...
  '00000000-0000-0000-0001-000000000005',  -- Citi PremierMiles
  '00000000-0000-0000-0001-000000000003',  -- UOB PRVI Miles Visa
  '00000000-0000-0000-0001-000000000009',  -- UOB PRVI Miles Amex
  '00000000-0000-0000-0001-000000000018',  -- UOB PRVI Miles Mastercard
...
-- ── UOB Visa Signature (...012): hybrid, contactless-only (no online whitelist) ──
-- Contactless earns on all MCCs except the exclusions (same list as Preferred
-- Platinum; petrol 5541/5542 not excluded → earns). Online/chip earn base.
...
-- ── Maybank XL Rewards (...016): whitelist (Dine / Shop / Travel / Play) ──────
...
-- ── Maybank Horizon (...008): whitelist ──────────────────────────────────────
-- MCC 8699 earns only with the Diamond Sky Fuel Card (noted). Merchant-name
-- exclusions (BAGUS, INSTAREM, SIMPLYGO, SINGTEL DASH, …) are not MCC-modellable.
...
-- ── Citi PremierMiles (...005): blacklist (excluded MCCs) ────────────────────
...
-- ── UOB PRVI Miles trio (...003 Visa / ...009 Amex / ...018 Mastercard): blacklist ──
...
-- ── DBS Altitude (...001): blacklist (excluded MCCs) ─────────────────────────
...
-- ── Amex KrisFlyer Ascend (...019): blacklist (only MCC-mappable exclusions) ──
-- Amex publishes no MCC table; SPC/petrol NOT excluded (only the SPC merchant is).
...
-- ── OCBC 90°N (...006): blacklist (excluded MCCs) ────────────────────────────
...
-- ── HSBC TravelOne (...007): blacklist (excluded MCCs) ───────────────────────
