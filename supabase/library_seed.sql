...
-- ─────────────────────────────────────────────────────────────────────────────
-- MCC eligibility (consolidated from migrations 044–051)
-- Requires the mcc_mode column (migration 046) and card_mcc_eligibility table
-- (migration 044). Seeded here too so a fresh install has MCC data without
-- depending on the data migrations' run order (those set mcc_mode by name and
-- no-op if run before the cards are inserted). Extra MCC descriptions live in
-- mcc_seed.sql. Idempotent: mode set by id, rows replaced per card.
-- ─────────────────────────────────────────────────────────────────────────────
